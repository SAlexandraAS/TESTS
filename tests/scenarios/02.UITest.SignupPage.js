...
const setDefaultValueForFormExclde = (excludeField, page) => {
  ...
  excludeField !== 'term' && page.clickElement('@agreeSignupTerm');
...
module.exports = {
  ...
  'Signup is ok': async (browser) => {
    ...
    page.clickElement('@agreeSignupTerm');
    page.submit();
    browser.pause(5000);
    ...
    browser.click('#inboxpane table tr td:nth-child(3n)');
    ...
    browser.pause(1000);
    const linkInEmail = await browser.getAttribute('.cuerpo a[target="_other"]', 'href');
  ...
  'Signup with disagree with the term': async (browser) => {
    const page = browser.page.signuppage();
    setDefaultValueForFormExclde('term', page).submit();
    browser.assert.not.urlContains('sign-up-welcome', 'The submit form has been not submitted');
  },
};
